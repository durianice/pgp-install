...
start_docker () {
    ...
    case $PGM_WEB in
        true)
            docker run -dit --restart=always --name="$container_name" --hostname="$container_name" -e WEB_ENABLE="$PGM_WEB" -e WEB_SECRET_KEY="$admin_password" -e WEB_HOST=0.0.0.0 -e WEB_PORT=3333-e WEB_LOGIN="$PGM_WEB_LOGIN" -p ${external_port}:3333 teampgm/pagermaid_pyro <&1
            ;;
        *)
        ...
        case $persistence in
            [yY][eE][sS] | [yY])
                ...
                if docker inspect "$container_name" &>/dev/null; then
                    ...
                    case $PGM_WEB in
                        true)
                            docker run -dit -v "$data_path"/workdir:/pagermaid/workdir --restart=always --name="$container_name" --hostname="$container_name" -e WEB_ENABLE="$PGM_WEB" -e WEB_SECRET_KEY="$admin_password" -e WEB_HOST=0.0.0.0 -e WEB_PORT=3333 -p ${external_port}:3333 teampgm/pagermaid_pyro <&1
                            ;;
                        *)
